<?php
/**
 * This file contains the MailTrillApi_Http_Request class used in the MailTrillApi PHP-SDK.
 *
...
/**
 * MailTrillApi_Http_Request is the request class used to send the requests to the API endpoints.
 *
 * @author Muzammil Versiani <muzammil.versiani@mailtrill.com>
 ...
 * @since 1.0
 */
class MailTrillApi_Http_Request extends MailTrillApi_Base
{
    /**
     * @var MailTrillApi_Http_Client the http client injected.
     */
    public $client;

    /**
     * @var MailTrillApi_Params the request params.
     */
    public $params;
     ...
     * Constructor.
     *
     * @param MailTrillApi_Http_Client $client
     */
    public function __construct(MailTrillApi_Http_Client $client)
    {
        $this->client = $client;
    }
    ...
    public function send()
    {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT , 'MailTrillApi Client version '. MailTrillApi_Http_Client::CLIENT_VERSION);
        ...
        if ($client->isPostMethod || $client->isPutMethod || $client->isDeleteMethod) {
            ...
            $params = new MailTrillApi_Params($client->paramsPost);
        ...
        $params = $this->params = new MailTrillApi_Params($curlInfo);
        ...
        if ($curlCode === 0 && $client->getResponseHeaders) {
            ...
            $params->add('headers', new MailTrillApi_Params($headers));
        }

        $decodedBody = array();
        if ($curlCode === 0 && !empty($body)) {
            $decodedBody = MailTrillApi_Json::decode($body, true);
        ...
        $params->add('body', new MailTrillApi_Params($decodedBody));

        $response = new MailTrillApi_Http_Response($this);
        $body = $response->body;
    ...
    protected function sign($requestUrl)
    {
        ...
        $params = new MailTrillApi_Params($specialHeaderParams);
