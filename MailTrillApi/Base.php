<?php
/**
 * This file contains the base class for the MailTrillApi PHP-SDK.
 *
 * @author Muzammil Versiani <muzammil.versiani@mailtrill.com>
 * @link http://www.mailtrill.com/
 * @copyright 2018 http://www.mailtrill.com/
 */
 
 
/**
 * MailTrillApi_Base is the base class for all the other classes used in the sdk.
 * 
 * @author Muzammil Versiani <muzammil.versiani@mailtrill.com>
 * @package MailTrillApi
 * @since 1.0
 */
class MailTrillApi_Base
{
    /**
    ...
    /**
     * @var MailTrillApi_Config the configuration object injected into the application at runtime  
     */
    private static $_config;
    
    /**
     * @var MailTrillApi_Params the package registry that will hold various components
     */
    private static $_registry = array();
    
    /**
     * @var MailTrillApi_Params the registered event handlers
     */
     ...
     * 
     * @param MailTrillApi_Config $config
     */
    public static function setConfig(MailTrillApi_Config $config)
    {
        self::$_config = $config;
    }
     ...
     * 
     * @return MailTrillApi_Params
     */
    public function getRegistry()
    {
        if (!(self::$_registry instanceof MailTrillApi_Params)) {
            self::$_registry = new MailTrillApi_Params(self::$_registry);
        }
     ...
     * $components = array(
     *     'cache'=>array(
     *         'class'             => 'MailTrillApi_Cache_Sqlite',
    ...
    public function setComponent($componentName, array $config)
    {
        ...
        if ($component instanceof MailTrillApi_Base) {
    ...
    {
        if (!(self::$_eventHandlers instanceof MailTrillApi_Params)) {
            self::$_eventHandlers = new MailTrillApi_Params(self::$_eventHandlers);
        }
        
        if (!self::$_eventHandlers->contains($eventName) || !(self::$_eventHandlers->itemAt($eventName) instanceof MailTrillApi_Params)) {
            self::$_eventHandlers->add($eventName, new MailTrillApi_Params());
        }
